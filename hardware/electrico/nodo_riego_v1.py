...
def caja(d, x, y, w, h, titulo, izq=(), der=(), stub=0.6, fs=10 * FS, color=INK,
         titulo_fs=11 * FS, titulo_loc='top', sub=None, titulo2=None):
...
with schemdraw.Drawing(file=SALIDA, show=False) as d:
    ...
    # Título y subtítulo
    texto(d, -0.5, 18.4, 'Nodo de riego v1 — ESP32 + sensores + relés (Fase 1)', fs=16 * FS)
    texto(d, -0.5, 17.75,
          'Rack de microgreens: riego por histéresis, ventilación, luces T8 y nivel de tinaco. '
          'Firmware: firmware/esphome/nodo-riego-v1.yaml', fs=10 * FS, color=GRIS)

    # ---------------- ESP32 DevKit V1 (30 pines) ----------------
    X0, Y0, W, H = 11.0, 2.5, 5.6, 10.6
    ...
    texto(d, X0 + W / 2, Y0 + H + 0.65, 'ESP32 DevKit V1 (30 pines) · ESPHome', fs=12 * FS, halign='center')
    esp = {}
    ...
    texto(d, X0 + W / 2, Y0 - 0.85, 'USB: solo flasheo inicial (después OTA).', fs=9 * FS,
          halign='center', color=GRIS)
    texto(d, X0 + W / 2, Y0 - 1.2, 'No conectar USB y VIN a la vez.', fs=9 * FS,
          halign='center', color=AMBAR)

    # 3V3 y GND del ESP32
    d.add(elm.Vdd().at(esp['v33']).color(VERDE).label('3V3 (≤ 600 mA)', fontsize=9 * FS, color=VERDE))
    ...
    def sensor(nombre, cy, titulo, salidas, vcc='3V3', sub=None, titulo2=None):
        a = caja(d, SX, cy - SH / 2, SW, SH, titulo,
                 izq=[(vcc, 'vcc'), ('GND', 'gnd')], der=salidas, fs=9.5 * FS, titulo_fs=10 * FS, sub=sub,
                 titulo2=titulo2)
    ...
    d.add(elm.Resistor().at((6.5, centros['ds'])).up().length(1.1).label('4.7 kΩ', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Vdd().at((6.5, centros['ds'] + 1.1)).color(VERDE).label('3V3', fontsize=9 * FS, color=VERDE))

    # JSN-SR04T: TRIG directo; ECHO es de 5 V -> divisor 10 k / 20 k a 3.3 V
    ruta(d, jsn['trig'], esp['g5'], xm=8.5)
    ye = jsn['echo'][1]
    d.add(elm.Resistor().at(jsn['echo']).right().length(1.8).label('10 kΩ', fontsize=9 * FS, loc='bottom'))
    nodo = (jsn['echo'][0] + 1.8, ye)
    punto(d, nodo)
    d.add(elm.Resistor().at(nodo).down().length(1.3).label('20 kΩ', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Ground().at((nodo[0], ye - 1.3)))
    ruta(d, nodo, esp['g18'], xm=9.0)
    texto(d, 9.15, ye + 0.15, 'ECHO 5 V → 3.3 V', fs=8.5 * FS, color=GRIS)

    # ---------------- Módulo relé 4 canales ----------------
    RX, RY, RW, RH = 18.0, 4.4, 3.6, 8.5
    rect(d, (RX, RY), (RX + RW, RY + RH))
    texto(d, RX + RW / 2, RY + RH + 0.28, 'Módulo relé 4 ch 5 V optoacoplado', fs=11 * FS, halign='center')
    texto(d, RX + RW / 2, RY - 0.35, 'activas en BAJO (IN = 0 → K cerrado) · JD-VCC = VCC', fs=8.5 * FS, halign='center', color=GRIS)
    texto(d, RX + RW / 2, RY - 0.7, 'contactos 10 A / 250 V CA', fs=8.5 * FS, halign='center', color=GRIS)
    rel = {}
    for nom, etq in [('vin', 'VCC 5 V'), ('gndr', 'GND'), ('g25', 'IN1'), ('g26', 'IN2'),
                     ('g27', 'IN3'), ('g14', 'IN4')]:
        py = esp[nom][1]
        d.add(elm.Line().at((RX, py)).left(0.6))
        texto(d, RX + 0.12, py, etq, fs=9.5 * FS)
        rel[nom] = (RX - 0.6, py)
    ...
    for i, nom in enumerate(['com1', 'no1', 'com2', 'no2', 'com3', 'no3', 'com4', 'no4']):
        ...
        texto(d, RX + RW - 0.12, py, nom.upper(), fs=9.5 * FS, halign='right')
    ...
    def carga_12v(com, no, fus, etiqueta, nota):
        d.add(elm.Vdd().at(com).color(VERDE).label('+12 V', fontsize=9 * FS, color=VERDE))
        d.add(elm.Fuse().at(no).right().length(1.6).label(fus, fontsize=9 * FS).color(VERDE))
        d.add(elm.Motor().at((no[0] + 1.6, no[1])).right().length(1.6))
        texto(d, no[0] + 2.4, no[1] + 0.8, etiqueta, fs=9 * FS, halign='center')
        d.add(elm.Ground().at((no[0] + 3.2, no[1])))
        texto(d, no[0] + 3.7, no[1] - 0.3, nota, fs=8.5 * FS, color=GRIS)
    ...
    d.add(elm.Vdd().at(cont['com3']).color(VERDE).label('+12 V', fontsize=9 * FS, color=VERDE))
    d.add(elm.Fuse().at(cont['no3']).right().length(1.6).label('F3 1 A', fontsize=9 * FS).color(VERDE))
    bob = (cont['no3'][0] + 1.6, cont['no3'][1])
    d.add(elm.Inductor2().at(bob).right().length(1.6))
    texto(d, bob[0] + 0.8, bob[1] + 0.9, 'K5 bobina 12 V', fs=9 * FS, halign='center')
    d.add(elm.Ground().at((bob[0] + 1.6, bob[1])))
    texto(d, bob[0] + 2.1, bob[1] - 0.3, 'relé de potencia / contactor 127 V', fs=8.5 * FS, color=GRIS)
    # Contacto de K5 en el circuito de 127 V (gabinete separado, ámbar)
    yl = 2.4
    xl = 22.6
    texto(d, xl + 1.6, yl + 1.35, 'L 127 V CA (del contacto GFCI, gabinete de CA)', fs=9 * FS, color=AMBAR)
    d.add(elm.Line().at((xl, yl)).right(0.5).color(AMBAR))
    d.add(elm.Fuse().at((xl + 0.5, yl)).right().length(1.5).label('F5 2 A', fontsize=9 * FS, color=AMBAR).color(AMBAR))
    d.add(elm.Switch().at((xl + 2.0, yl)).right().length(1.5).label('K5', fontsize=9 * FS, loc='bottom', color=AMBAR).color(AMBAR))
    d.add(elm.Lamp().at((xl + 3.5, yl)).right().length(1.5).label('T8 LED 18 W ×8 = 144 W',
                                                                fontsize=9 * FS, loc='bottom', color=AMBAR).color(AMBAR))
    d.add(elm.Line().at((xl + 5.0, yl)).right(0.5).color(AMBAR))
    texto(d, xl + 5.6, yl, 'N', fs=9 * FS, color=AMBAR)
    ...
    texto(d, xl - 0.3, yl - 1.3, 'gabinete 127 V CA separado · tierra física al chasis metálico', fs=8.5 * FS, color=AMBAR)

    # K4 reserva
    d.add(elm.Vdd().at(cont['com4']).color(VERDE).label('+12 V', fontsize=9 * FS, color=VERDE))
    d.add(elm.Line().at(cont['no4']).right(1.2).linestyle('--').color(GRIS))
    texto(d, cont['no4'][0] + 1.4, cont['no4'][1] + 0.2, 'K4 reserva: tapete térmico 127 V (dic–feb)', fs=9 * FS, color=GRIS)
    texto(d, cont['no4'][0] + 1.4, cont['no4'][1] - 0.2, 'o luz nivel 5; mismo esquema que K3', fs=9 * FS, color=GRIS)

    # ---------------- Alimentación (arriba a la derecha) ----------------
    PX, PY = 16.2, 14.6
    gf = caja(d, PX, PY, 3.0, 1.6, 'Contacto GFCI 127 V', der=[('L', 'l'), ('N', 'n'), ('T', 't')],
              fs=9 * FS, titulo_fs=10 * FS, color=AMBAR)
    texto(d, PX + 1.5, PY + 0.8, 'tapa "in-use"', fs=8.5 * FS, halign='center', color=GRIS)
    FX = PX + 4.4
    fu = caja(d, FX, PY, 3.4, 1.6, 'Fuente Steren ELI-1260 12 V 5 A',
              izq=[('L', 'l'), ('N', 'n'), ('T', 't')], der=[('+12 V', 'p'), ('GND', 'g')], fs=9 * FS, titulo_fs=10 * FS)
    for k in ['l', 'n', 't']:
        d.add(elm.Line().at(gf[k]).to(fu[k]).color(AMBAR))
    # +12 V -> fusible general -> nodo +12 V y buck
    d.add(elm.Fuse().at(fu['p']).right().length(1.6).label('F0 5 A', fontsize=9 * FS, loc='bottom').color(VERDE))
    n12 = (fu['p'][0] + 1.6, fu['p'][1])
    punto(d, n12, color=VERDE)
    d.add(elm.Vdd().at(n12).color(VERDE).label('+12 V (a K1–K4)', fontsize=9 * FS, color=VERDE))
    ...
    texto(d, BX + 1.4, BY - 0.3, 'ajustar 5.0 V', fs=8.5 * FS, halign='center', color=GRIS)
    ...
    texto(d, 19.0, y5 + 0.3, '+5 V → VIN ESP32, VCC módulo relé, JSN-SR04T', fs=9 * FS, color=VERDE)
    ...
    texto(d, -0.5, 16.25, 'Sensores con el conector hacia arriba y sellado.', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 16.65, 'ADC1 (GPIO32–35) para los capacitivos: ADC2 no funciona con Wi-Fi activo.',
          fs=9.5 * FS, color=GRIS)
    ...
    texto(d, LX + 1.2, LY + 2.3, 'alimentación CC (12 V · 5 V · 3V3)', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.8)).right(0.8).color(AMBAR))
    texto(d, LX + 1.2, LY + 1.8, '127 V CA — solo dentro del gabinete de CA, aguas abajo del GFCI', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.3)).right(0.8))
    texto(d, LX + 1.2, LY + 1.3, 'señal / control 3.3 V', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 0.8)).right(0.8).linestyle('--').color(GRIS))
    texto(d, LX + 1.2, LY + 0.8, 'enlace mecánico / opcional', fs=9.5 * FS)
    texto(d, LX + 0.2, LY + 0.3, 'F = fusible por rama · K = relé · valores de fusible protegen el cable, no la carga', fs=9.5 * FS)

    texto(d, -0.5, -3.3, 'Fuentes: research/electronica-automatizacion.md · research/electrico-respaldo-seguridad.md · referencia/03-instalacion.md §1.3', fs=9 * FS, color=GRIS)
    texto(d, -0.5, -3.8, f'{VERSION} · HomeGreen · generado con schemdraw desde hardware/electrico/nodo_riego_v1.py', fs=9 * FS, color=GRIS)
