...
with schemdraw.Drawing(file=SALIDA, show=False) as d:
    ...
    texto(d, -0.5, 17.6, 'Nodo NFT v2 — pH/EC, dosificación y continuidad (Fase 2)', fs=16 * FS)
    texto(d, -0.5, 16.95,
          'Alimentado desde el bus de batería (bus-dc-first.svg): sigue midiendo y avisando durante un corte de CFE. '
          'Firmware: firmware/esphome/nodo-nft-v2.yaml', fs=10 * FS, color=GRIS)
    texto(d, -0.5, 16.2, 'GPIO34/35/36 son solo entrada (sin pull-up interno). GPIO12 es pin de arranque (strapping): '
          'pull-down 10 kΩ a GND obligatorio o el ESP32 no arranca.', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 15.75, 'Sondas pH y TDS en el RETORNO del NFT; peristálticas dosifican al depósito junto a la succión de la bomba. '
          'Filtro RC 1 kΩ / 100 nF en cada entrada ADC.', fs=9.5 * FS, color=GRIS)

    # ---------------- ESP32 ----------------
    X0, Y0, W, H = 10.2, 1.0, 6.7, 13.0
    paso = H / 10
    ys = [Y0 + H - paso * (i + 1) for i in range(9)]
    rect(d, (X0, Y0), (X0 + W, Y0 + H))
    texto(d, X0 + W / 2, Y0 + H + 0.28, 'ESP32 DevKit V1 (30 pines) · ESPHome', fs=12 * FS, halign='center')
    ...
    for (etq, nom), py in zip(izq, ys):
        ...
        texto(d, X0 + 0.12, py, etq, fs=9.5 * FS)
        esp[nom] = (X0 - 0.6, py)
    for (etq, nom), py in zip(der, ys):
        d.add(elm.Line().at((X0 + W, py)).right(0.6))
        texto(d, X0 + W - 0.12, py, etq, fs=9.5 * FS, halign='right')
        esp[nom] = (X0 + W + 0.6, py)
    d.add(elm.Line().at((X0 + W / 2, Y0)).down(0.3))
    # En un solo renglón el rótulo mide ~10.4 u y sobresale 1.8 u por cada lado
    # del ESP32: por la izquierda le entraba el cable verde de 3V3 del
    # optoacoplador (x ≈ 8.65). En dos renglones cabe dentro del ancho del chip
    # y queda por encima del riel +12 V del canal típico (y ≈ −0.3).
    texto(d, X0 + W / 2, Y0 - 0.55, 'USB: solo flasheo inicial (después OTA).',
          fs=8.5 * FS, halign='center', color=AMBAR)
    texto(d, X0 + W / 2, Y0 - 0.95, 'No conectar USB y VIN a la vez.',
          fs=8.5 * FS, halign='center', color=AMBAR)
    d.add(elm.Vdd().at(esp['v33']).color(VERDE).label('3V3 (≤ 600 mA)', fontsize=9 * FS, color=VERDE, ofst=(0, 0.35)))
    ...
    def sensor(cy, titulo, salidas, vcc='3V3', titulo2=None, abajo=()):
        a = caja(d, SX, cy - SH / 2, SW, SH, titulo, izq=[(vcc, 'vcc'), ('GND', 'gnd')], der=salidas,
                 abajo=abajo, fs=9.5 * FS, titulo_fs=10 * FS, titulo2=titulo2)
        d.add(elm.Vdd().at(a['vcc']).color(VERDE).label(vcc, fontsize=9 * FS, color=VERDE, loc='left'))
        d.add(elm.Ground().at(a['gnd']))
        return a

    def filtro_rc(a, b, xm):
        """Serie 1 kΩ + 100 nF a GND antes del ADC."""
        d.add(elm.Resistor().at(a).right().length(1.4).label('1 kΩ', fontsize=9 * FS))
        n = (a[0] + 1.4, a[1])
        punto(d, n)
        d.add(elm.Capacitor().at(n).down().length(1.0).label('100 nF', fontsize=9 * FS, loc='bottom'))
        d.add(elm.Ground().at((n[0], n[1] - 1.0)))
        ruta(d, n, b, xm=xm)

    # pH
    ph = sensor(13.6, 'Placa pH PH-4502C (o DFRobot Gravity)', [('Po', 'po')], vcc='5V',
                abajo=[('BNC', 'bnc')], titulo2='Po ≤ 3.1 V: ajustar offset con buffer 4.01')
    d.add(elm.Dot(open=True, radius=0.16).at(ph['bnc']))
    texto(d, ph['bnc'][0] + 0.35, ph['bnc'][1] + 0.3, 'BNC ← electrodo E201 (retorno)', fs=8.5 * FS, color=GRIS)
    ...
    texto(d, tds['sd'][0] + 0.35, tds['sd'][1], 'sonda TDS (retorno)', fs=8.5 * FS, color=GRIS)
    texto(d, tds['sd'][0] + 0.35, tds['sd'][1] - 0.35, 'calibrar en mS/cm', fs=8.5 * FS, color=GRIS)
    filtro_rc(tds['ao'], esp['g35'], xm=8.1)

    # Divisor de voltaje de batería 47k/10k -> GPIO36 (x5.7 en firmware)
    xv, yv = 0.6, 8.0
    d.add(elm.Vdd().at((xv, yv + 1.2)).color(VERDE).label('BUS 12.8 V (F3)', fontsize=9 * FS, color=VERDE))
    d.add(elm.Resistor().at((xv, yv + 1.2)).down().length(1.2).label('47 kΩ', fontsize=9 * FS, loc='left'))
    punto(d, (xv, yv))
    d.add(elm.Resistor().at((xv, yv)).down().length(1.2).label('10 kΩ', fontsize=9 * FS, loc='left'))
    d.add(elm.Ground().at((xv, yv - 1.2)))
    d.add(elm.Line().at((xv, yv)).to((6.1, yv)))
    punto(d, (6.1, yv))
    d.add(elm.Capacitor().at((6.1, yv)).down().length(1.0).label('100 nF', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Ground().at((6.1, yv - 1.0)))
    ruta(d, (6.1, yv), esp['g36'], xm=7.6)
    texto(d, 1.6, yv + 0.55, 'Divisor V_bat: 14.6 V → 2.56 V', fs=8.5 * FS, color=GRIS)
    texto(d, 1.6, yv + 0.2, '(×5.7 en firmware; atenuación 12 dB)', fs=8.5 * FS, color=GRIS)

    # DS18B20
    ds = sensor(5.2, 'DS18B20 · T solución (depósito)', [('DQ', 'dq')])
    ruta(d, ds['dq'], esp['g4'], xm=7.0)
    punto(d, (6.5, 5.2))
    d.add(elm.Resistor().at((6.5, 5.2)).up().length(1.1).label('4.7 kΩ', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Vdd().at((6.5, 6.3)).color(VERDE).label('3V3', fontsize=9 * FS, color=VERDE))

    # YF-S201 (salida Hall a 5 V -> divisor 10k/20k)
    yf = sensor(2.4, 'YF-S201 · flujo NFT (≈450 pulsos/L)', [('SIG', 'sig')], vcc='5V')
    d.add(elm.Resistor().at(yf['sig']).right().length(1.4).label('10 kΩ', fontsize=9 * FS))
    n = (yf['sig'][0] + 1.4, yf['sig'][1])
    punto(d, n)
    d.add(elm.Resistor().at(n).down().length(1.2).label('20 kΩ', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Ground().at((n[0], n[1] - 1.2)))
    ruta(d, n, esp['g27'], xm=7.5)
    texto(d, 7.7, 2.65, 'SIG 5 V → 3.3 V', fs=8.5 * FS, color=GRIS)
    ...
    usb = caja(d, 0.6, -1.3, 3.2, 1.4, 'Cargador USB 5 V al contacto GFCI',
               der=[('+5 V', 'p'), ('GND_USB', 'g')], fs=9.5 * FS, titulo_fs=10 * FS,
               titulo2='detector de red CFE: NO unir GND_USB al nodo')
    d.add(elm.Resistor().at(usb['p']).right().length(1.3).label('470 Ω', fontsize=9 * FS))
    ...
    oc = ((opto.anode[0] + opto.collector[0]) / 2, min(opto.cathode[1], opto.emitter[1]) - 0.62)
    texto(d, oc[0], oc[1], 'PC817', fs=9 * FS, halign='center', color=GRIS)
    d.add(elm.Line().at(opto.collector).up(0.4))
    d.add(elm.Vdd().at((opto.collector[0], opto.collector[1] + 0.4)).color(VERDE).label('3V3', fontsize=9 * FS, color=VERDE))
    ne = (opto.emitter[0] + 0.6, opto.emitter[1])
    d.add(elm.Line().at(opto.emitter).to(ne))
    punto(d, ne)
    d.add(elm.Resistor().at(ne).down().length(1.2).label('10 kΩ', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Ground().at((ne[0], ne[1] - 1.2)))
    ruta(d, ne, esp['g23'], xm=8.0)
    texto(d, 8.2, opto.emitter[1] + 0.35, 'con 127 V presente → GPIO23 = 1', fs=8.5 * FS, color=GRIS)

    # ---------------- Salidas (derecha) ----------------
    # Driver MOSFET x5 (canal típico detallado abajo)
    DX, DY, DW, DH = 18.0, 4.4, 3.4, 6.2
    rect(d, (DX, DY), (DX + DW, DY + DH))
    texto(d, DX + DW / 2, DY + DH + 0.28, 'Driver MOSFET ×5', fs=11 * FS, halign='center')
    texto(d, DX + DW / 2, DY + DH + 0.28 + 0.34 * FS, '(canal típico abajo)', fs=8.5 * FS, halign='center', color=GRIS)
    texto(d, DX + DW / 2, DY + DH - 1.2, 'GND → barra común', fs=8 * FS, halign='center', color=GRIS)
    ...
    for nom, ein, eout, tipo, etq in canales:
        ...
        texto(d, DX + 0.12, py, ein, fs=9.5 * FS)
        d.add(elm.Line().at(esp[nom]).to((DX - 0.6, py)))
        d.add(elm.Line().at((DX + DW, py)).right(0.6))
        texto(d, DX + DW - 0.12, py, eout, fs=9.5 * FS, halign='right')
        o = (DX + DW + 0.6, py)
        if tipo == 'motor':
            d.add(elm.Motor().at(o).right().length(1.8))
        else:
            d.add(elm.Inductor2().at(o).right().length(1.8))
        # El nombre del actuador va DESPUÉS del riel +12 V, no encima del
        # símbolo: entre dos filas solo quedan 0.3 u libres entre un círculo de
        # motor y el siguiente, y el rótulo ampliado ya no cabía ahí.
        texto(d, XR + 0.4, py + 0.24, etq, fs=8.5 * FS)
        d.add(elm.Line().at((o[0] + 1.8, py)).to((XR, py)).color(VERDE))
        punto(d, (XR, py), color=VERDE)
    # pull-down obligatorio en GPIO12 (strapping)
    g12 = esp['g12']
    punto(d, (g12[0] + 0.2, g12[1]))
    d.add(elm.Resistor().at((g12[0] + 0.2, g12[1])).down().length(1.0).label('10 kΩ', fontsize=8.5 * FS, loc='bottom').color(AMBAR))
    d.add(elm.Ground().at((g12[0] + 0.2, g12[1] - 1.0)))
    d.add(elm.Line().at((XR, esp['g13'][1])).to((XR, esp['g25'][1] + 0.5)).color(VERDE))
    d.add(elm.Vdd().at((XR, esp['g25'][1] + 0.5)).color(VERDE).label('+12 V · F4 5 A (fusiblera)', fontsize=9 * FS, color=VERDE))
    texto(d, XR + 0.4, esp['g25'][1] - 0.24, 'A y B: sales; pH−: AquAcid', fs=8.5 * FS, color=GRIS)
    texto(d, XR + 0.4, esp['g12'][1] - 0.24, 'NC: sin energía = cerrada', fs=8.5 * FS, color=GRIS)
    texto(d, XR + 0.4, esp['g13'][1] - 0.24, '(un corte no vacía el tinaco)', fs=8.5 * FS, color=GRIS)

    # Relé 2 ch para K1 (principal, NC) y K2 (respaldo, NO) del bus DC
    RX, RY, RW, RH = 18.0, 1.7, 3.4, 2.2
    rect(d, (RX, RY), (RX + RW, RY + RH))
    # entre el ESP32 (x = 16.9) y la caja del driver (y = 4.4) hay 5.6 u de
    # ancho y 0.5 u de alto: el título largo cruzaba los dos bordes.
    texto(d, RX + RW / 2, RY + RH + 0.20, 'Relé 2 ch · bombas NFT', fs=9 * FS, halign='center')
    for nom, ein, eout, etq1, etq2 in [
        ...
        texto(d, RX + 0.12, py, ein, fs=9.5 * FS)
        d.add(elm.Line().at(esp[nom]).to((RX - 0.6, py)))
        d.add(elm.Line().at((RX + RW, py)).right(0.6))
        texto(d, RX + RW - 0.12, py, eout, fs=9.5 * FS, halign='right')
        d.add(elm.Line().at((RX + RW + 0.6, py)).right(1.0).linestyle('--').color(GRIS))
        texto(d, RX + RW + 1.8, py + 0.2, etq1, fs=8.5 * FS, color=GRIS)
        texto(d, RX + RW + 1.8, py - 0.2, etq2, fs=8.5 * FS, color=GRIS)
    texto(d, RX + RW + 1.8, RY - 0.1, 'contactos y fusibles de bomba: ver bus-dc-first.svg', fs=8.5 * FS, color=GRIS)
    d.add(elm.Line().at((RX + 1.0, RY)).down(0.5))
    d.add(elm.Vdd().at((RX + 1.0, RY - 0.5)).color(VERDE).label('5V', fontsize=9 * FS, color=VERDE).flip())
    texto(d, RX + 1.0, RY + 0.22, 'VCC', fs=9 * FS, halign='center')
    d.add(elm.Line().at((RX + 2.4, RY)).down(0.5))
    d.add(elm.Ground().at((RX + 2.4, RY - 0.5)))
    texto(d, RX + 2.4, RY + 0.22, 'GND', fs=9 * FS, halign='center')

    # ---------------- Alimentación (arriba a la derecha) ----------------
    PX, PY = 18.0, 14.2
    d.add(elm.Vdd().at((PX, PY)).color(VERDE).label('BUS 12.8 V', fontsize=9 * FS, color=VERDE))
    texto(d, PX - 0.2, PY + 1.1, 'desde la fusiblera del bus DC, F3 (bus-dc-first.svg)', fs=8.5 * FS, halign='right', color=GRIS)
    d.add(elm.Fuse().at((PX, PY)).right().length(1.6).label('F3 2 A', fontsize=9 * FS, loc='bottom').color(VERDE))
    n12 = (PX + 1.6, PY)
    bk = caja(d, n12[0] + 0.8, PY - 0.9, 4.0, 1.6, 'Buck LM2596 12→5 V',
              izq=[('IN+', 'ip'), ('IN−', 'im')], der=[('OUT+', 'op'), ('OUT−', 'om')], fs=9 * FS, titulo_fs=10 * FS)
    texto(d, n12[0] + 2.8, PY - 0.1, 'ajustar 5.0 V', fs=8.5 * FS, halign='center', color=GRIS)
    ...
    d.add(elm.Capacitor().at(n5).down().length(1.2).label('470 µF', fontsize=9 * FS, loc='bottom').color(VERDE))
    ...
    texto(d, 17.2, esp['vin'][1] + 0.3, '+5 V → VIN ESP32, VCC relé 2 ch, placa pH, YF-S201', fs=9 * FS, color=VERDE)
    d.add(elm.Ground().at(esp['gndr']))

    # ---------------- Canal típico del driver ----------------
    CX, CY = 12.0, -4.4
    texto(d, CX, CY - 2.2, 'Canal típico del driver (×5): MOSFET de nivel lógico IRLZ44N o módulo D4184', fs=10 * FS)
    texto(d, CX, CY + 0.15, 'IN (GPIO)', fs=9 * FS)
    d.add(elm.Resistor().at((CX + 1.3, CY)).right().length(1.4).label('220 Ω', fontsize=9 * FS))
    g = (CX + 2.7, CY)
    punto(d, g)
    d.add(elm.Resistor().at(g).down().length(1.2).label('10 kΩ', fontsize=9 * FS, loc='bottom'))
    ...
    texto(d, q.drain[0] - 0.7, q.drain[1] + 0.8, 'carga 12 V', fs=9 * FS, halign='right')
    ...
    texto(d, q.drain[0] + 1.6, q.drain[1] + 0.8, 'D 1N5819 (flyback)', fs=9 * FS)
    d.add(elm.Line().at(top).up(0.3).color(VERDE))
    d.add(elm.Vdd().at((top[0], top[1] + 0.3)).color(VERDE).label('+12 V (F4)', fontsize=9 * FS, color=VERDE))

    # ---------------- Leyenda y pie ----------------
    LX, LY = -0.5, -6.6
    rect(d, (LX, LY), (LX + 12.0, LY + 3.2), color=GRIS)
    texto(d, LX + 0.2, LY + 2.85, 'Leyenda', fs=10.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 2.3)).right(0.8).color(VERDE))
    texto(d, LX + 1.2, LY + 2.3, 'alimentación CC desde el bus de batería (12.8 V · 5 V · 3V3)', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.8)).right(0.8))
    texto(d, LX + 1.2, LY + 1.8, 'señal / control 3.3 V', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.3)).right(0.8).linestyle('--').color(GRIS))
    texto(d, LX + 1.2, LY + 1.3, 'contacto de relé / referencia a otro esquema', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 0.8)).right(0.8).color(AMBAR))
    texto(d, LX + 1.2, LY + 0.8, 'componente obligatorio de seguridad (pull-down GPIO12)', fs=9.5 * FS)
    texto(d, LX + 0.2, LY + 0.3, 'F = fusible (fusiblera del bus) · K = relé · NC/NO = reposo cerrado/abierto', fs=9.5 * FS)

    texto(d, -0.5, -7.5, 'Fuentes: research/electrico-respaldo-seguridad.md §5 · research/electronica-automatizacion.md §3 · referencia/03-instalacion.md §2.2–2.3', fs=9 * FS, color=GRIS)
    texto(d, -0.5, -8.0, f'{VERSION} · HomeGreen · generado con schemdraw desde hardware/electrico/nodo_nft_v2.py', fs=9 * FS, color=GRIS)
