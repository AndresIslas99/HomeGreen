...
with schemdraw.Drawing(file=SALIDA, show=False) as d:
    ...
    texto(d, -0.5, 14.4, 'Alimentación y protección del ESP32 (común a los nodos de riego y NFT)', fs=16 * FS)
    texto(d, -0.5, 13.75, '12 V → buck → 5 V → ESP32 (3V3 interno) · capacitores de desacoplo · tierra común en estrella · presupuesto de corriente.',
          fs=10 * FS, color=GRIS)
    texto(d, -0.5, 13.3, 'Detalle del bloque "alimentación" de nodo-riego-v1.svg y nodo-nft-v2.svg.', fs=10 * FS, color=GRIS)

    # ---------------- Cadena 12 V -> 5 V ----------------
    y = 10.0
    d.add(elm.Vdd().at((0.0, y)).color(VERDE))
    texto(d, -0.3, y + 0.85, '+12 V', fs=9 * FS, halign='right', color=VERDE)
    texto(d, -0.5, y + 2.4, 'Entrada: +12 V desde la fusiblera del bus DC (F3) o desde la fuente 12 V 5 A (F0).', fs=9.5 * FS, color=GRIS)
    d.add(elm.Fuse().at((0.0, y)).right().length(1.8).color(VERDE).label('F 2 A', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Diode().at((1.8, y)).right().length(1.8).color(VERDE)
          .label('D1 1N5822 Schottky (polaridad inversa)', fontsize=9 * FS, loc='top'))
    na = (3.6, y)
    punto(d, na, color=VERDE)
    d.add(elm.Capacitor(polar=True).at(na).down().length(1.6))
    texto(d, na[0] - 0.35, na[1] - 1.1, 'C1 100 µF / 25 V', fs=9 * FS, halign='right')
    ...
    texto(d, nt[0] + 0.35, nt[1] - 1.9, 'TVS SMBJ15A (opcional:', fs=8.5 * FS, color=GRIS)
    texto(d, nt[0] + 0.35, nt[1] - 2.25, 'picos al arrancar la bomba)', fs=8.5 * FS, color=GRIS)
    bk = caja(d, 6.2, y - 0.8, 3.2, 1.6, 'Buck LM2596 (módulo 3 A)', izq=[('IN+', 'ip'), ('IN−', 'im')],
              der=[('OUT+', 'op'), ('OUT−', 'om')], titulo_fs=10 * FS, fs=9 * FS,
    ...
    texto(d, nc[0] + 0.35, nc[1] - 0.8, 'C2 470 µF / 10 V', fs=9 * FS)
    ...
    texto(d, ndd[0] + 0.35, ndd[1] - 0.8, 'C3 100 nF', fs=9 * FS)
    d.add(elm.Ground().at((ndd[0], ndd[1] - 1.6)))
    d.add(elm.Line().at(ndd).right(1.6).color(VERDE))
    # Rama +5 V a módulos
    d.add(elm.Line().at(ndd).up(1.5).color(VERDE))
    d.add(elm.Vdd().at((ndd[0], ndd[1] + 1.5)).color(VERDE).label('+5 V', fontsize=9 * FS, color=VERDE))
    texto(d, ndd[0] + 0.8, ndd[1] + 2.35, '→ módulo relé (VCC y JD-VCC), JSN-SR04T, placa pH, YF-S201', fs=9 * FS, color=VERDE)
    texto(d, ndd[0] + 0.8, ndd[1] + 2.0, 'cable cal. 18 AWG, < 30 cm del buck al ESP32 (evita brownout por caída)', fs=8.5 * FS, color=GRIS)

    # ---------------- ESP32 ----------------
    EX, EY, EW, EH = ndd[0] + 2.25, y5 - 3.7, 4.6, 4.7
    rect(d, (EX, EY), (EX + EW, EY + EH))
    texto(d, EX + EW / 2, EY + EH + 0.28, 'ESP32 DevKit V1', fs=12 * FS, halign='center')
    texto(d, EX + EW / 2, y5 - 0.6, 'AMS1117-3.3 (LDO interno)', fs=9 * FS, halign='center', color=GRIS)
    texto(d, EX + EW / 2, y5 - 0.95, '≤ 600 mA; disipa (5−3.3 V)·I', fs=8.5 * FS, halign='center', color=GRIS)
    texto(d, EX + EW / 2, EY + 0.48, 'USB: solo flasheo inicial;', fs=8.5 * FS, halign='center', color=AMBAR)
    texto(d, EX + EW / 2, EY + 0.06, 'nunca USB y VIN a la vez', fs=8.5 * FS, halign='center', color=AMBAR)
    pins_izq = [('VIN (5 V)', y5), ('GND', y5 - 1.5), ('EN', y5 - 2.7)]
    pins_der = [('3V3', y5), ('GND', y5 - 1.5), ('GPIOs (3.3 V)', y5 - 2.7)]
    esp = {}
    for etq, py in pins_izq:
        d.add(elm.Line().at((EX, py)).left(0.6))
        texto(d, EX + 0.12, py, etq, fs=9.5 * FS)
        esp['i_' + etq.split()[0]] = (EX - 0.6, py)
    for etq, py in pins_der:
        d.add(elm.Line().at((EX + EW, py)).right(0.6))
        texto(d, EX + EW - 0.12, py, etq, fs=9.5 * FS, halign='right')
    ...
    texto(d, esp['i_EN'][0] - 0.3, esp['i_EN'][1] - 0.5, 'C4 100 nF', fs=9 * FS, halign='right')
    d.add(elm.Ground().at((esp['i_EN'][0], esp['i_EN'][1] - 1.0)))
    texto(d, esp['i_EN'][0] + 0.2, esp['i_EN'][1] - 1.75, 'C4 en EN: reset limpio tras huecos', fs=8.5 * FS, halign='right', color=GRIS)
    texto(d, esp['i_EN'][0] + 0.2, esp['i_EN'][1] - 2.1, 'de la fuente (arranques fallidos)', fs=8.5 * FS, halign='right', color=GRIS)
    # 3V3 a sensores
    ...
    texto(d, ne[0] + 0.35, ne[1] - 0.8, 'C5 10 µF', fs=9 * FS)
    ...
    texto(d, nf[0] + 0.35, nf[1] - 0.8, 'C6 100 nF', fs=9 * FS)
    d.add(elm.Ground().at((nf[0], nf[1] - 1.6)))
    d.add(elm.Line().at(nf).right(1.8).color(VERDE))
    d.add(elm.Vdd().at((nf[0] + 1.8, y5)).color(VERDE).label('3V3', fontsize=9 * FS, color=VERDE))
    texto(d, nf[0] + 2.3, y5 + 0.45, '→ SHT31, DS18B20,', fs=9 * FS, color=VERDE)
    texto(d, nf[0] + 2.3, y5 + 0.05, 'capacitivos ×4, TDS, pull-ups', fs=9 * FS, color=VERDE)
    texto(d, nf[0] + 2.3, y5 - 0.35, 'solo sensores (decenas de mA);', fs=8.5 * FS, color=GRIS)
    texto(d, nf[0] + 2.3, y5 - 0.75, 'nunca relés/bombas', fs=8.5 * FS, color=GRIS)
    texto(d, nf[0] + 2.3, y5 - 1.15, '100 nF junto a cada sensor;', fs=8.5 * FS, color=GRIS)
    texto(d, nf[0] + 2.3, y5 - 1.55, 'conector hacia arriba', fs=8.5 * FS, color=GRIS)
    d.add(elm.Dot(radius=0.01).at((nf[0] + 6.0, y5)).color('#ffffff'))  # extiende el lienzo para el texto
    texto(d, EX, EY - 0.4, 'GPIOs: 3.3 V máx., ≤ 12 mA por pin → siempre a través de módulo relé / MOSFET', fs=8.5 * FS, color=GRIS)

    # ---------------- Tierra común ----------------
    texto(d, -0.5, 4.6, 'Tierra común en ESTRELLA', fs=11 * FS)
    texto(d, -0.5, 4.15, 'Todos los GND (fuente/fusiblera −BUS, buck, ESP32, módulo relé, drivers, sensores)', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 3.75, 'llegan a UNA barra dentro del gabinete de CC.', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 3.35, 'Retornos de potencia (bomba, peristálticas) por su propio cable a la barra: nunca por el cable de señal de un sensor.', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 2.95, 'El GND_USB del detector de red CFE queda aislado (optoacoplador).', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, 2.55, 'La tierra física (T, 127 V) va al chasis de los gabinetes, no a esta barra.', fs=9.5 * FS, color=GRIS)

    # ---------------- Presupuesto de corriente ----------------
    PX, PY = -0.5, -1.5
    rect(d, (PX, PY), (PX + 13.6, PY + 3.7), color=GRIS)
    texto(d, PX + 0.2, PY + 3.35, 'Presupuesto de corriente en 5 V (típicos de hoja de datos; medir en banco, V3)', fs=10 * FS)
    filas = ['ESP32 con Wi-Fi activo: 0.08–0.24 A (picos de transmisión)',
             ...
             'en 12 V: bomba diafragma 40 W = 3.3 A por F1 (fusiblera),',
             'ventilador por F2, NO por el buck']
    for i, t in enumerate(filas):
        texto(d, PX + 0.4, PY + 2.85 - i * 0.55, '• ' + t, fs=9 * FS)

    # ---------------- Protecciones ----------------
    QX, QY = 14.0, -1.5
    rect(d, (QX, QY), (QX + 15.0, QY + 3.7), color=AMBAR, ls='--')
    texto(d, QX + 0.2, QY + 3.35, 'Protecciones de este bloque (checklist antes de energizar)', fs=10 * FS, color=AMBAR)
    ...
    for i, t in enumerate(prot):
        texto(d, QX + 0.4, QY + 2.85 - i * 0.55, '• ' + t, fs=9 * FS)

    # ---------------- Leyenda y pie ----------------
    LX, LY = 20.0, 3.3
    rect(d, (LX, LY), (LX + 9.0, LY + 2.2), color=GRIS)
    texto(d, LX + 0.2, LY + 1.85, 'Leyenda', fs=10.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.35)).right(0.8).color(VERDE))
    texto(d, LX + 1.2, LY + 1.35, 'alimentación CC (12 V · 5 V · 3V3)', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 0.85)).right(0.8).linestyle('--').color(GRIS))
    texto(d, LX + 1.2, LY + 0.85, 'opcional', fs=9.5 * FS)
    texto(d, LX + 0.2, LY + 0.35, 'C polarizados: + al lado del riel positivo', fs=9.5 * FS)

    texto(d, -0.5, -2.1, 'Fuentes: referencia/03-instalacion.md §1.3 · research/electronica-automatizacion.md §2 · research/electrico-respaldo-seguridad.md §5', fs=9 * FS, color=GRIS)
    texto(d, -0.5, -2.6, f'{VERSION} · HomeGreen · generado con schemdraw desde hardware/electrico/alimentacion_esp32.py', fs=9 * FS, color=GRIS)
