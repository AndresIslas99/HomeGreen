...
with schemdraw.Drawing(file=SALIDA, show=False) as d:
    ...
    texto(d, -0.5, 17.6, 'Bus DC-first — continuidad de la bomba NFT ante cortes de CFE (Fase 2)', fs=16 * FS)
    texto(d, -0.5, 16.95, 'Topología de DC-UPS: la batería está SIEMPRE en paralelo con el cargador; el corte de CFE no se conmuta, no se nota. '
          'El fail-safe es físico, no de software.', fs=10 * FS, color=GRIS)
    texto(d, -0.5, 16.2, 'Autonomía (fuente): bomba 12 V de 40 W = 3.3 A → 12 h = 480 Wh ≈ 38–40 Ah. LiFePO4 100 Ah al 90 % DoD ≈ 28–30 h continuas; '
          'modo 15 min ON / 15 min OFF ≈ 2.5 días.', fs=9.5 * FS, color=GRIS)
    ...
    texto(d, -0.5, 14.9, 'En casa (otro circuito): UPS DataShield DS-600 → router + cerebro HA. Sin internet no hay alarmas; el agua NO depende de HA.',
          fs=9.5 * FS, color=AMBAR)

    # ---------------- Lado CA (ámbar) ----------------
    yca = 12.6
    texto(d, -0.5, yca + 0.45, 'CFE 127 V CA', fs=10 * FS, color=AMBAR)
    d.add(elm.Line().at((-0.5, yca)).right(2.1).color(AMBAR))
    d.add(elm.Breaker().at((1.6, yca)).right().length(2.0).color(AMBAR)
          .label('QO120GFI 20 A (GFCI) · centro de carga', fontsize=8.5 * FS, loc='bottom', color=AMBAR))
    d.add(elm.Line().at((3.6, yca)).right(1.0).color(AMBAR))
    ct = caja(d, 4.6, yca - 0.8, 3.2, 1.6, 'Contacto WR + tapa "in-use"', izq=[('', 'i')], der=[('', 'o')],
              color=AMBAR, titulo_fs=10 * FS, sub='patio · intemperie')
    d.add(elm.Line().at(ct['o']).right(0.6).color(AMBAR))
    cg = caja(d, 9.0, yca - 0.8, 3.6, 1.6, 'Cargador LiFePO4',
              izq=[('CA', 'ca')], der=[('B+', 'bp'), ('B−', 'bm')], color=INK, titulo_fs=10 * FS,
              titulo2='14.6 V · 10–20 A')
    texto(d, 8.6, 11.35, 'absorción 14.4–14.6 V · flotación ≤ 13.6 V', fs=8.5 * FS, halign='center', color=GRIS)
    ...
    texto(d, FX + FW / 2, FY + FH + 0.28, 'Fusiblera 6 vías 12 V', fs=11 * FS, halign='center')
    texto(d, FX + FW / 2, FY + FH + 0.28 + 0.34 * FS, '(gabinete IP65 de CC)', fs=8.5 * FS, halign='center', color=GRIS)
    d.add(elm.Line().at((FX, 8.0)).left(0.6).color(VERDE))
    texto(d, FX + 0.12, 8.0, '+BUS', fs=9.5 * FS)
    ...
    texto(d, FX + FW / 2, FY + 0.22, '−BUS', fs=9.5 * FS, halign='center')
    texto(d, FX + FW / 2 + 0.5, FY - 0.9, 'barra negativa común (batería, cargador, controlador, cargas)', fs=8.5 * FS, color=GRIS)
    ...
    texto(d, -0.3, 8.25, 'OPCIONAL (Fase 2.5, +$1,650 aprox.): el controlador hace de cargador; un corte diurno se vuelve sostenible', fs=8.5 * FS, color=GRIS)
    d.add(elm.Solar().at((1.15, ysol)).right().length(2.0).label('Panel 100 W Ugreen 15113 (19 V)', fontsize=9 * FS, loc='bottom'))
    ls = caja(d, 4.0, ysol - 0.8, 4.8, 1.6, 'EPEVER LS2024B PWM 20 A',
              izq=[('PV+', 'pvp'), ('PV−', 'pvm')], der=[('BAT+', 'bp'), ('BAT−', 'bm')], titulo_fs=10 * FS,
              sub='perfil LiFePO4')
    d.add(elm.Line().at((3.15, ysol)).to(ls['pvp']).color(VERDE))
    ruta(d, (1.15, ysol), ls['pvm'], xm=1.15, color=INK)
    ...
    texto(d, bx - 0.45, by + 2.2, '+', fs=12 * FS)
    texto(d, bx - 0.45, by + 0.05, '−', fs=12 * FS)
    texto(d, bx + 0.5, by + 1.55, 'LiFePO4 12.8 V 100 Ah', fs=10 * FS)
    texto(d, bx + 0.5, by + 1.15, 'Epcom LI100A12PRO ($4,459)', fs=9 * FS, color=GRIS)
    texto(d, bx + 0.5, by + 0.8, 'BMS interno (corte por bajo V)', fs=8.5 * FS, color=GRIS)
    ...
    d.add(elm.Fuse().at((bx, by + 3.0)).right().length(2.0).color(VERDE)
          .label('F0 30 A (ANL/MIDI)', fontsize=9 * FS, loc='top'))
    texto(d, bx + 0.3, by + 3.0 - 0.35, '≤ 30 cm del borne +', fs=8.5 * FS, color=GRIS)
    d.add(elm.Switch().at((bx + 2.0, by + 3.0)).right().length(2.0).color(VERDE))
    texto(d, bx + 3.0, by + 3.0 + 0.72, 'S1 seccionador (mantenimiento)', fs=9 * FS, halign='center', color=VERDE)
    ruta(d, (bx + 4.0, by + 3.0), NB, xm=12.0, color=VERDE)
    punto(d, (12.0, 8.0), color=VERDE)
    texto(d, 6.2, 5.6, 'SIEMPRE en paralelo: cero conmutación', fs=9.5 * FS, color=VERDE)
    texto(d, 6.2, 5.2, 'HA cicla la bomba 15/15 min si V_bat < 12.9 V en corte', fs=8.5 * FS, color=GRIS)
    ...
    for i, (etq, nom) in enumerate(salidas):
        ...
        texto(d, FX + FW - 0.12, py, etq.split()[0], fs=9.5 * FS, halign='right')
        d.add(elm.Fuse().at((FX + FW + 0.5, py)).right().length(1.6).color(VERDE).label(etq, fontsize=9 * FS, loc='top'))
    XO = FX + FW + 2.1  # fin de cada fusible

    # Bombas con relevadores de transferencia K1 (NC) y K2 (NO)
    y1 = ysal['bomba1']
    d.add(elm.Switch(nc=True).at((XO, y1)).right().length(1.8).label('K1 (NC)', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Motor().at((XO + 1.8, y1)).right().length(1.8).label('Bomba principal 12 V 40–60 W · 24/7', fontsize=9 * FS, loc='top'))
    d.add(elm.Ground().at((XO + 3.6, y1)))
    texto(d, XO + 4.1, y1 - 0.05, 'sin ESP32 o GPIO32 = 0 → bomba ON', fs=8.5 * FS, color=GRIS)
    texto(d, XO + 4.1, y1 - 0.45, 'bobinas K1/K2 ← relé 2 ch del nodo NFT; contactos ≥ 10 A', fs=8.5 * FS, color=GRIS)
    y2 = ysal['bomba2']
    d.add(elm.Switch().at((XO, y2)).right().length(1.8).label('K2 (NO)', fontsize=9 * FS, loc='bottom'))
    d.add(elm.Motor().at((XO + 1.8, y2)).right().length(1.8).label('Bomba respaldo 12 V (idéntica)', fontsize=9 * FS, loc='top'))
    d.add(elm.Ground().at((XO + 3.6, y2)))
    texto(d, XO + 4.1, y2 - 0.05, 'GPIO14 = 1 → respaldo ON (sin flujo en YF-S201)', fs=8.5 * FS, color=GRIS)

    # Nodo NFT
    y3 = ysal['nodo']
    rect(d, (XO + 0.6, y3 - 0.6), (XO + 5.0, y3 + 0.6))
    texto(d, XO + 2.8, y3 - 0.92, 'Nodo NFT ESP32 + buck 5 V (nodo-nft-v2.svg)', fs=10 * FS, halign='center')
    d.add(elm.Line().at((XO + 0.6, y3)).left(0.6).color(VERDE))
    texto(d, XO + 0.72, y3, '+12.8 V', fs=9.5 * FS)
    d.add(elm.Line().at((XO + 5.0, y3)).right(0.6))
    texto(d, XO + 4.88, y3, 'GND', fs=9.5 * FS, halign='right')
    d.add(elm.Ground().at((XO + 5.6, y3)))
    texto(d, XO + 6.2, y3 + 0.2, 'siempre vivo: mide V_bat, flujo y red CFE;', fs=8.5 * FS, color=GRIS)
    texto(d, XO + 6.2, y3 - 0.2, 'manda K1/K2 y avisa a HA', fs=8.5 * FS, color=GRIS)

    # Actuadores 12 V
    y4 = ysal['actuadores']
    d.add(elm.Line().at((XO, y4)).right(0.8).color(VERDE))
    texto(d, XO + 1.0, y4 - 0.1, '→ actuadores 12 V del nodo NFT (driver MOSFET):', fs=9 * FS)
    texto(d, XO + 1.0, y4 - 0.5, '3 peristálticas A/B/pH− + 2 solenoides ½" NC', fs=9 * FS)

    # Reserva y libre
    y5 = ysal['reserva']
    d.add(elm.Line().at((XO, y5)).right(0.8).linestyle('--').color(GRIS))
    texto(d, XO + 1.0, y5, '→ reserva: nodo de riego v1 si se migra al bus, o luz de servicio 12 V', fs=9 * FS, color=GRIS)
    y6 = ysal['libre']
    d.add(elm.Line().at((XO, y6)).right(0.8).linestyle('--').color(GRIS))
    texto(d, XO + 1.0, y6, '→ libre', fs=9 * FS, color=GRIS)
    ...
    texto(d, -0.5, 0.1, 'Instalación: todo el 12 V en gabinete IP65 separado del de 127 V · cables de bomba cal. 14 AWG, nodo cal. 18 AWG · '
          'F0 protege el cable de batería; F1–F6 protegen cada rama.', fs=9.5 * FS, color=GRIS)
    texto(d, -0.5, -0.35, 'Variante austera de arranque (fuente): Steren BR-1224 24 Ah + cargador BR-700 + 1 bomba + contacto GFCI ≈ $2,500–2,900 → cortes ≤ 7 h; '
          'se amplía a 100 Ah sin tirar nada.', fs=9.5 * FS, color=GRIS)
    ...
    # ---------------- Leyenda y pie ----------------
    LX, LY = 20.0, -4.4
    rect(d, (LX, LY), (LX + 11.4, LY + 3.2), color=GRIS)
    texto(d, LX + 0.2, LY + 2.85, 'Leyenda', fs=10.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 2.35)).right(0.8).color(VERDE))
    texto(d, LX + 1.2, LY + 2.35, 'bus 12.8 V CC (batería flotada)', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.85)).right(0.8).color(AMBAR))
    texto(d, LX + 1.2, LY + 1.85, '127 V CA aguas abajo del GFCI', fs=9.5 * FS)
    d.add(elm.Line().at((LX + 0.2, LY + 1.35)).right(0.8).linestyle('--').color(GRIS))
    texto(d, LX + 1.2, LY + 1.35, 'opcional / reserva', fs=9.5 * FS)
    texto(d, LX + 0.2, LY + 0.85, 'F = fusible · K = relé · NC/NO = reposo cerrado/abierto', fs=9.5 * FS)
    texto(d, LX + 0.2, LY + 0.35, 'S = seccionador', fs=9.5 * FS)

    texto(d, -0.5, -3.7, 'Fuentes: research/electrico-respaldo-seguridad.md §2–3, §6 · referencia/03-instalacion.md §2.3 · bom/fase2.csv (respaldo)', fs=9 * FS, color=GRIS)
    texto(d, -0.5, -4.2, f'{VERSION} · HomeGreen · generado con schemdraw desde hardware/electrico/bus_dc_first.py', fs=9 * FS, color=GRIS)
